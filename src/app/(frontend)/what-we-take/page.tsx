...
import { getItemCategories, getWhatWeTakeSettings } from "@/lib/payload-data";
...
export default async function WhatWeTakePage() {
  const [itemCategories, whatWeTake] = await Promise.all([getItemCategories(), getWhatWeTakeSettings()]);

  const graph = schemaGraph([
    breadcrumbNode("/what-we-take", [
      { name: "Home", path: "/" },
      { name: "What We Take", path: "/what-we-take" },
    ]),
    itemListNode(
      "/what-we-take",
      itemCategories.map((g) => ({ name: g.title, path: `/what-we-take/${g.slug}` })),
    ),
  ]);
      ...
      <Section>
        <div className="grid gap-5 sm:grid-cols-2">
          {itemCategories.map((group) => (
            <Link
              key={group.slug}
              href={`/what-we-take/${group.slug}`}
              className="rounded-2xl border border-border bg-card p-6 shadow-soft transition-smooth hover:-translate-y-1 hover:border-primary/40 hover:shadow-lift"
            >
              <span
      ...
      <Section tone="sand">
        <div className="rounded-2xl border border-destructive/25 bg-card p-7 shadow-soft">
          ...
          <ul className="mt-5 grid gap-2 text-sm text-foreground sm:grid-cols-2">
            {whatWeTake.notAccepted.map((item) => (
              <li key={item} className="flex gap-2">
